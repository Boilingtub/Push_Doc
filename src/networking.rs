            ...
            Some(index1) => {
                ...
                match url[index1 + 3..].find("/") {
                    ...
                    None => {
                        ...
                        return Err(format!(
                            "{:?} is invalid url , url need at learst 1 \"/\" after domain",
                            url
                        ))?;
                    }
                }
...
fn ip_address_resolution_error_escape(domain: &str, e: Error) {
    eprintln!(
        "Could not find corrosponding Ip Addres for\n 
                {}:443\nERROR={}\n 
...
pub fn send_https(
    ...
    let mut content = method.to_string()
        + " "
        + &url.path
        + " HTTP/1.1\r\n"
        + "Host: "
...
pub async fn listen_https(addr: SocketAddr, response: &str) -> String {
    ...
    let certs = load_certs_from_str(CERT_CONTENT)
        .expect("Included certificate is not valid, Or could not be found !");
    let key = load_key_from_str(KEY_CONTENT)
        .expect("Included Private_Key is not valid, Or could not be found !");
