use crate::networking::{
    generate_random_data_base64url, get_random_unused_port, listen_https, send_https,
};
use crate::validate;
use std::fs;
...
impl ClientSecrets {
    pub fn new(client_secrets: &str) -> Result<ClientSecrets, String> {
        ...
        let token_uri = client_secrets_json["installed"]["token_uri"]
            .as_str()
            .expect("Could not find \"token_uri\" in client_secrets_json");
        let client_id = client_secrets_json["installed"]["client_id"]
            .as_str()
            .expect("Could not find \"client_id\" in client_secrets_json");
        let client_secret = client_secrets_json["installed"]["client_secret"]
            .as_str()
            .expect("Could not find \"client_secret\" in client_secrets_json");
        let redirect_uris = client_secrets_json["installed"]["redirect_uris"]
            .as_str()
    ...
    pub fn to_string(&self) -> String {
        format!(
            "ClientSecrets {{ token_uri: {} client_id: {} client_secret: {} redirect_uris: {} }}",
...
pub async fn recurse_async_get_access_token(renew: bool, client_secrets: &ClientSecrets) -> String {
    Box::pin(async move { get_access_token(renew, client_secrets).await }).await
}
...
pub async fn get_access_token(renew: bool, client_secrets: &ClientSecrets) -> String {
    if validate::if_token_exists() == true {
        let token_raw_json = match fs::read_to_string("auth/token") {
            Ok(v) => v,
            Err(e) => {
                eprintln!("Could not read token file, Err:{}", e);
                panic!();
            }
        };
        let token_json = match jsonic::parse(&token_raw_json) {
            Ok(v) => v,
            Err(..) => {
                eprintln!("Token file not valid json, recreating...");
        ...
        let token_type = match token_json["token_type"].as_str() {
            Some(v) => v,
            None => {
                eprintln!("Could not find `token_type`, recreating token...");
                create_access_token(client_secrets).await;
                return recurse_async_get_access_token(renew, client_secrets).await;
            }
        };

        let to_get = if renew == false {
            "access_token"
        } else {
            "refresh_token"
        };
        let token_value = match token_json[to_get].as_str() {
            Some(v) => v,
            None => {
                eprintln!("Could not find `access_token`, recreating token...");
                create_access_token(client_secrets).await;
                return recurse_async_get_access_token(renew, client_secrets).await;
            }
        };
...
pub fn get_client_secrets() -> ClientSecrets {
    ...
    let client_secrets_raw_json = match fs::read_to_string(path) {
        ...
        Err(e) => {
            eprintln!(
                "Could not read client_secrets file\n{}
    ...
    match ClientSecrets::new(&client_secrets_raw_json) {
        ...
        Err(e) => {
            eprintln!(
                "Please reinstall your client_secrets file\n 
                            {}\n",
                e
            );
            std::process::exit(1);
...
#[allow(unused)]
pub async fn exchange_code_for_tokens_async(
...
fn parse_auth_code_from_response(httpresponse: &str) -> String {
    ...
    let index1 = match httpresponse.find("code=") {
        Some(v) => v,
        None => {
            eprintln!("there is no code in googleauth URI");
    ...
    let index2 = match httpresponse.find("&scope") {
        Some(v) => v,
        None => {
            eprintln!("there is no code in googleauth Scope");
            return "Invalid google response".to_string();
        }
    };
...
async fn update_auth_token_local(renew_token: &str, client_secrets: &ClientSecrets) {
    let renew_json = match jsonic::parse(renew_token) {
        ...
        Err(e) => {
            println!(
                "failed to parse, renew Token, not valid json\n
    ...
    let token_raw_json = match std::fs::read_to_string("auth/token") {
        Ok(v) => v,
        Err(e) => {
            println!("Could not read token file, Err:{}", e);
            panic!();
        }
    };
    ...
    let token_json = match jsonic::parse(&token_raw_json) {
        ...
        Err(e) => {
            println!(
                "failed to parse,  Token file, not valid json\n
                    Try restarting the application\n{}",
                e
            );
            std::process::exit(1);
    ...
    let org_access_token = token_json["access_token"]
        .as_str()
        .expect("invalid JSON , could not find `access_token field` in org_access_token");
    let new_access_token = renew_json["access_token"]
        .as_str()
    ...
    let org_token_str = match token_json.as_str() {
        ...
        None => {
            println!(
                "FATAL ERROR : token_json invalid json ,cannot be converted to
                        string"
            );
            std::process::exit(1);
        }
